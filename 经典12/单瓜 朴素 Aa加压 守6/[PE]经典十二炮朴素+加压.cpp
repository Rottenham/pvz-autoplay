...
Stat total_level_stat("总"), N_level_stat("N"), N_pos_stat("N位置"),
    smart_a_level_stat("加压a"), smart_A_level_stat("加压A");

void on_fail();

void smart_squash(int wave) { // 窝瓜加压
    ...
    int best_row = std::distance(count.begin(), std::max_element(count.begin(), count.end())) + 1;
    At(now + 182) a(best_row, 9);
    smart_a_level_stat[to_string(level)]++;
}

void smart_cherry(int wave) { // 樱桃加压
    std::array<int, 6> count = {};
    for (auto& z : aliveZombieFilter) {
        if (z.AtWave() + 1 == wave) {
            if (z.Type() == GIGA) count[z.Row()] += 18;
...
void Script()
{
    ...
    SelectCards({LILY, DOOM, ICE, COFFEE, SQUASH, CHERRY, 40, 41, 42, 43}, 0);
    ...
    for (int w = 1; w <= 18; w++) {
        if (w == 9 || w == 10) continue;
        OnWave(w) { 
            At(341) PP(),
            At(402) Do {
                if (NowWave() == w && GetMainObject()->RefreshCountdown() > 200) {
                    if (IsSeedUsable(SQUASH)) smart_squash(w);
                    else if (IsSeedUsable(CHERRY)) smart_cherry(w);
                    else smart_A_level_stat["failed"]++;
                }
            }
        };
    }
    OnWave(9, 19) At(341) PP();
    OnWave(10) {
        At(409) PP(9.2),
        At(830 - 299) Do {
...
OnBeforeScript({
    if (flag_count > 0 && flag_count % 100 == 0) {
        logger.Debug("#, #\n#, #\n#, #", N_level_stat, N_pos_stat,
                                         smart_a_level_stat, smart_A_level_stat,
